...
class Highway(tf.keras.models.Model):
    ...
    def build(self, input_shape):
        ...
        self.dense_g = tfa.layers.SpectralNormalization(Dense(output_dim, activation="relu"),4)
        self.dense_t = tfa.layers.SpectralNormalization(Dense(output_dim, activation="sigmoid"),4)
...
class Discriminator:
    ...
    def __init__(
        ...
        for filter_size, num_filter in zip(filter_sizes, num_filters):
            x = tfa.layers.SpectralNormalization(Conv1D(num_filter, filter_size),4)(layer_emb)
